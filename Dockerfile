...
RUN rpm --import https://ast.tucny.com/repo/RPM-GPG-KEY-dtucny && yum -y install wget && (cd /tmp; wget http://dl.fedoraproject.org/pub/epel/7/x86_64/e/epel-release-7-5.noarch.rpm; rpm -ivh /tmp/epel-release-7-5.noarch.rpm) && yum -y install asterisk asterisk-fax asterisk-sip && yum clean all -y
